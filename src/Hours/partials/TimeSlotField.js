...
export const timeSlotHandlers = {
  ...
  handleStartTimeFieldChange: (props) => (event, currentValue) => {
    const { hoursFields, fieldIndex, fieldName, meta, change, dayIndex } =
      props
  ...
  /* istanbul ignore next */
  handleEndTimeFieldChange: (props) => (event, currentValue) => {
    const { hoursFields, fieldIndex, fieldName, meta, change, dayIndex } =
      props
    const closeDay = getCloseDay(
      dayIndex,
        ...
        <Flex pb={3} pt={3} alignItems="center">
          <Box width={1 / 2} pr={1}>
            <Field
              ...
              format={(event) =>
                timeSlotHandlers.handleFormatTime(props)(event)
              }
          ...
          {hoursFields.get(fieldIndex) === hoursFields.getAll().first() ? (
            <Box width={[1 / 8, 1 / 16]} pl={1} style={{ textAlign: 'right' }}>
              <FlatButton
